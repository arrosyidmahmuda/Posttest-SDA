...
bool cariBuku(BukuNode *head, string judulDicari)
{
    ...
    if (head == nullptr)
    {
        return false; // 1. list kosong, kembali ke false
    }

    BukuNode *temp = head; // 2. penelusuran dari head menggunakan pointer temp

    do
    {
        if (temp->judulBuku == judulDicari)
        {
            return true; // 3. jika temp->judulBuku sama dengan judulDicari, langsung kembalikan true
        }
        temp = temp->next; // 4. lanjutkan ke node berikutnya
